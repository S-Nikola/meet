...
export const getEvents = async () => {
  ...
  if (token) {
    ...
    const result = await /*fetch*/axios.get(url);
    if (result.data) {
...
export const getAccessToken = async () => {
  ...
  if (!accessToken || tokenCheck.error) {
    ...
    if (!code) {
      const results = await /*fetch*/axios.get(
...
const getToken = async (code) => {
  try {
      ...
      const response = await fetch( 'https://gu5frrhod4.execute-api.eu-central-1.amazonaws.com/dev/api/token' + '/' + encodeCode);
      if (!response.ok) {
